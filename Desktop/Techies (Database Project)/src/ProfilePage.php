...
if(!isset($_SESSION['mem_id']))
	...
	$result_mem_info = pg_query($db_connection, "SELECT * from member_info where mem_id='{$own_mem_id}'") or die('cannot see table records');
    ...
    $result_profile = pg_query($db_connection, "SELECT * from profile where mem_id='$own_mem_id'") or die('cannot see table records');
...
<script type="text/javascript">
	$(document).ready(function(){
        loadprofilepic();
        $('#ProfileTextUploadForm').submit(function(event){
         $.ajax({
                type: 'POST',
             ...
             cache:false,
             processData:false,
                url: 'setprofiletext.php',
             success: function(data) {
                    //alert(data);
                    //$("#message").html(data);
                    //$('#message').css("display", "block");
                    
                    $('#profileText').val(data);
                }
            });
	   return false;	 
     });	 
	 $('#ProfileDetailsUpdateForm').submit(function(event){
         $.ajax({
                type: 'POST',
                data: new FormData(this),
                contentType:false,
             ...
             success: function(data) {
                    ...
                    $('#phone_num').val(data['phone_no']);
                    $('#line1Add').val(data['l1address']);
                    $('#line2Add').val(data['l2address']);
                    $('#city').val(data['city']);
...
<li><a href="Messages.php">MESSAGES</a></li>
  ...
  <section class="profileHeader">    
   ...
   <textarea style="width:500px;float:left;height:80px;word-wrap:normal;" type="text" id="profileText" name="profileText" placeholder="Profile Text : Please enter the Profile Text here" ><?php echo $profilestring;?></textarea>
    </div>
    <button type="submit" id="updateProfileText" class="btn-sm" style="margin-top: 10px;text-color:black;">Update Profile Text</button>
    </form>
  </section>
  <hr>
  <!-- content -->
<section class="mainContent"> 
  ...
  <section class="section1">
    ...
    <div class="section1Content">
      ...
      <input id="dob" value="<?php echo $own_dob; ?>" readonly="true" type="date" style="background:#00CA79;border:none;width:160px;"/></br></p>
      <p><label style="float:left;margin-right:15px;width: 60px;" for="phone_num" id="l_phone" title="Phone :">Phone :</label>
      <input type="text" id="phone_num" name="phone_num" value="<?php echo $own_phone; ?>" /></br></p>
      <p><label style="float:left;margin-right:15px;width: 70px;" for="address" id="l_address" title="Address :" >Address :</label></br></p>
      <address id="address">
      <p><input type="text" id="line1Add" name="line1Add" value="<?php echo $own_l1address; ?>" placeholder="Line 2 Address" /></p>
      <p><input type="text" id="line2Add" name="line2Add" value="<?php echo $own_l2address; ?>" placeholder="Line 2 Address" /><br/></p>
      <p><input type="text" id="city" name="city" value="<?php echo $own_city; ?>" placeholder = "City" /></p>
      <p><input type="text" id="state" name="state" value="<?php echo $own_state; ?>" placeholder="State" /><br/></p>
      <input type="text" id="country" name="country" value="<?php echo $own_country; ?>" placeholder="Country" /> 
      <input type="text" id="zipcode" name="zipcode" value="<?php echo $own_zipcode; ?>" placeholder="ZipCode" /> 
      </address>
      <br>
      <button type="submit" id="updateProfile" name="updateProfile" class="btn-sm" style="margin-top: 10px;text-color:black;float:left;">Update Profiles</button>
    </div>
    </form>
  </section>
