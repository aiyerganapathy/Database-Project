...
$result_profile = pg_query($con, "SELECT * from profile where mem_id='$own_mem_id'") or die('cannot see table records');
$profile_multimedia_id = pg_fetch_result($result_profile,'multimedia_id_ref');
$query_latestphoto = "SELECT * FROM multimedia_details where multimedia_id={$profile_multimedia_id} and mem_id={$own_mem_id}";
